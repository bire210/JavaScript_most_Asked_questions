...
- A: `Birendra` and `undefined`
- B: `Birendra` and `ReferenceError`
- C: `ReferenceError` and `24`
...
The string `'Birendra'` is a truthy value. What we're actually asking, is "is this truthy value falsy?". This returns `false`.
...
const member = new Person('Birendra', 'Mahto');
...
const Birendra = new Person('Birendra', 'Mahto');
const Prity = Person('Prity', 'Kumari');

console.log(Birendra);
console.log(Prity);
```

- A: `Person {firstName: "Birendra", lastName: "Mahto"}` and `undefined`
- B: `Person {firstName: "Birendra", lastName: "Mahto"}` and `Person {firstName: "Prity", lastName: "Kumari"}`
- C: `Person {firstName: "Birendra", lastName: "Mahto"}` and `{}`
- D: `Person {firstName: "Birendra", lastName: "Mahto"}` and `ReferenceError`
...
For `Prity`, we didn't use the `new` keyword. When using `new`, `this` refers to the new empty object we create. However, if you don't add `new`, `this` refers to the **global object**!

We said that `this.firstName` equals `"Prity"` and `this.lastName` equals `"Kumari"`. What we actually did, is defining `global.firstName = 'Prity'` and `global.lastName = 'Kumari'`. `Prity` itself is left `undefined`, since we don't return a value from the `Person` function.
...
const person = 'Birendra';
const age = 24;

getPersonInfo`${person} is ${age} years old`;
```

- A: `"Birendra"` `24` `["", " is ", " years old"]`
- B: `["", " is ", " years old"]` `"Birendra"` `24`
- C: `"Birendra"` `["", " is ", " years old"]` `24`
...
String.prototype.giveBirendraPizza = () => {
  return 'Just give Birendra pizza already!';
};

const name = 'Birendra';

console.log(name.giveBirendraPizza())
```

- A: `"Just give Birendra pizza already!"`
...
- A: `undefined is 24` `Birendra is 24`
- B: `function` `function`
- C: `Birendra is 24` `Birendra is 24`
- D: `Birendra is 24` `function`
...
- A: `{ name: "Birendra" }, { age: 24 }`
- B: `"name", "age"`
- C: `"Birendra", 24`
- D: `["name", "Birendra"], ["age", 24]`
...
function getInfo(member, year) {
  member.name = 'Birendra';
  year = '1998';
}

const person = { name: 'Prity' };
...
- A: `{ name: "Birendra" }, "1997"`
- B: `{ name: "Prity" }, "1998"`
- C: `{ name: "Birendra" }, "1998"`
- D: `{ name: "Prity" }, "1997"`
...
The value of `person` is an object. The argument `member` has a (copied) reference to the _same_ object. When we modify a property of the object `member` has a reference to, the value of `person` will also be modified, since they both have a reference to the same object. `person`'s `name` property is now equal to the value `"Birendra"`
...
The `name` variable was declared with a `const` keyword, so its deletion is not successful: `false` is returned. When we set `age` equal to `24`, we actually added a property called `age` to the global object. You can successfully delete properties from objects this way, also the global object, so `delete age` returns `true`.
...
```javascript
const user = { name: 'Birendra', age: 24 };
const admin = { admin: true, ...user };

console.log(admin);
```

- A: `{ admin: true, user: { name: "Birendra", age: 24 } }`
- B: `{ admin: true, name: "Birendra", age: 24 }`
- C: `{ admin: true, user: ["Birendra", 24] }`
- D: `{ admin: true }`
...
It's possible to combine objects using the spread operator `...`. It lets you create copies of the key/value pairs of one object, and add them to another object. In this case, we create copies of the `user` object, and add them to the `admin` object. The `admin` object now contains the copied key/value pairs, which results in `{ admin: true, name: "Birendra", age: 24 }`.
...
- A: `{ name: "Birendra", age: 24 }`, `["name", "age"]`
- B: `{ name: "Birendra", age: 24 }`, `["name"]`
- C: `{ name: "Birendra"}`, `["name", "age"]`
- D: `{ name: "Birendra"}`, `["age"]`
...
- B: `"{"username": "BirendraMahto"}"`
- C: `"["level", "health"]"`
- D: `"{"username": "BirendraMahto", "level":19, "health":90}"`
...
```javascript
const name = 'Birendra Mahto';
console.log(name.padStart(13));
console.log(name.padStart(2));
```

- A: `"Birendra Mahto"`, `"Birendra Mahto"`
- B: `" Birendra Mahto"`, `" Birendra Mahto"` (`"[13x whitespace]Birendra Mahto"`, `"[2x whitespace]Birendra Mahto"`)
- C: `" Birendra Mahto"`, `"Birendra Mahto"` (`"[1x whitespace]Birendra Mahto"`, `"Birendra Mahto"`)
- D: `"Birendra Mahto"`, `"Lyd"`,
...
With the `padStart` method, we can add padding to the beginning of a string. The value passed to this method is the _total_ length of the string together with the padding. The string `"Birendra Mahto"` has a length of `12`. `name.padStart(13)` inserts 1 space at the start of the string, because 12 + 1 is 13.
...
```javascript
const { firstName: myName } = { firstName: 'Birendra' };
...
```javascript
const { firstName } = { firstName: 'Birendra' };
// ES5 version:
// var firstName = { firstName: 'Birendra' }.firstName;

console.log(firstName); // "Birendra"
```
...
const { firstName: myName } = { firstName: 'Birendra' };
// ES5 version:
// var myName = { firstName: 'Birendra' }.firstName;

console.log(myName); // "Birendra"
...
```javascript
const { name: myName } = { name: 'Birendra' };
...
`function sayHi(name = "Birendra") { ... }`

In this case, if we didn't pass a value or if we passed `undefined`, `name` would always be equal to the string `Birendra`
...
- A: `{ name: "Birendra", age: 24 }`
- B: `{ name: "Birendra", age: 24, city: "Amsterdam" }`
- C: `{ name: "Birendra", age: 24, city: undefined }`
- D: `"Amsterdam"`
...
- A: `{ default: function default(), name: "Birendra" }`
- B: `{ default: function default() }`
- C: `{ default: "Hello world", name: "Birendra" }`
...
```javascript
function giveBirendraPizza() {
  return 'Here is pizza!';
}

const giveBirendraChocolate = () =>
  "Here's chocolate... now go hit the gym already.";

console.log(giveBirendraPizza.prototype);
console.log(giveBirendraChocolate.prototype);
```
...
Regular functions, such as the `giveBirendraPizza` function, have a `prototype` property, which is an object (prototype object) with a `constructor` property. Arrow functions however, such as the `giveBirendraChocolate` function, do not have this `prototype` property. `undefined` gets returned when trying to access the `prototype` property using `giveBirendraChocolate.prototype`.
...
- A: `name` `Birendra` and `age` `24`
- B: `["name", "Birendra"]` and `["age", 24]`
...
`[ [ 'name', 'Birendra' ], [ 'age', 24 ] ]`

Using the `for-of` loop, we can iterate over each element in the array, the subarrays in this case. We can destructure the subarrays instantly in the for-of loop, using `const [x, y]`. `x` is equal to the first element in the subarray, `y` is equal to the second element in the subarray.

The first subarray is `[ "name", "Birendra" ]`, with `x` equal to `"name"`, and `y` equal to `"Birendra"`, which get logged.
The second subarray is `[ "age", 24 ]`, with `x` equal to `"age"`, and `y` equal to `24`, which get logged.
...
We can set classes equal to other classes/function constructors. In this case, we set `Person` equal to `AnotherPerson`. The name on this constructor is `Prity`, so the name property on the new `Person` instance `member` is `"Prity"`.
...
- B: `[1, [2, 3, 4]]` and `{ name: "Birendra", age: 24 }`
- C: `[1, 2, 3, 4]` and `{ name: "Birendra", age: 24 }`
- D: `Error` and `{ name: "Birendra", age: 24 }`
...
- A: `3`, `NaN`, `NaN`
- B: `3`, `7`, `NaN`
- C: `3`, `Birendra2`, `[object Object]2`
- D: `"12"`, `Birendra2`, `[object Object]2`
...
However, the second one is a string `"Birendra"`. `"Birendra"` is a string and `2` is a number: `2` gets coerced into a string. `"Birendra"` and `"2"` get concatenated, which results in the string `"Birendra2"`.

`{ name: "Birendra" }` is an object. Neither a number nor an object is a string, so it stringifies both. Whenever we stringify a regular object, it becomes `"[object Object]"`. `"[object Object]"` concatenated with `"2"` becomes `"[object Object]2"`.
...
In JavaScript, primitive data types (everything that's not an object) interact by _value_. In this case, we set the value of the `favoriteFood` property on the `info` object equal to the value of the first element in the `food` array, the string with the pizza emoji in this case (`'🍕'`). A string is a primitive data type, and interact by value (see my [blogpost](https://www.theavocoder.com/complete-javascript/2018/12/24/by-value-vs-by-reference) if you're interested in learning more)
...
const jsonArray = JSON.stringify({ name: 'Birendra' }); // '{"name":"Birendra"}'
JSON.parse(jsonArray); // { name: 'Birendra' }
```
...
Each function has its own _execution context_ (or _scope_). The `getName` function first looks within its own context (scope) to see if it contains the variable `name` we're trying to access. In this case, the `getName` function contains its own `name` variable: we declare the variable `name` with the `let` keyword, and with the value of `'Prity'`.

Variables with the `let` keyword (and `const`) are hoisted, but unlike `var`, don't get <i>initialized</i>. They are not accessible before the line we declare (initialize) them. This is called the "temporal dead zone". When we try to access the variables before they are declared, JavaScript throws a `ReferenceError`.

If we wouldn't have declared the `name` variable within the `getName` function, the javascript engine would've looked down the _scope chain_. The outer scope has a variable called `name` with the value of `Birendra`. In that case, it would've logged `Birendra`.
...
- A: `{name: "Prity", age: 22}`
- B: `{name: "Prity", age: 23}`
- C: `{name: "Birendra", age: 22}`
- D: `{name: "Birendra", age: 23}`
...
First, we invoke the `changeAge` function and pass the `person` object as its argument. This function increases the value of the `age` property by 1. `person` is now `{ name: "Birendra", age: 22 }`.

Then, we invoke the `changeAgeAndName` function, however we don't pass a parameter. Instead, the value of `x` is equal to a _new_ object: `{ ...person }`. Since it's a new object, it doesn't affect the values of the properties on the `person` object. `person` is still equal to `{ name: "Birendra", age: 22 }`.
...
- B: `Mara` `undefined` `Birendra Mahto` `ReferenceError`
- C: `Mara` `null` `Birendra Mahto` `null`
...
`person.getFullName?.()`: `person` has a property named `getFullName`: `person.getFullName()` is not nullish and can get invoked, which returns `Birendra Mahto`.
...
```javascript
const randomValue = 24;

function getInfo() {
  console.log(typeof randomValue);
  const randomValue = 'Birendra Mahto';
}
...
First, we invoke `funcOne`. On the first line of `funcOne`, we call the _asynchronous_ `setTimeout` function, from which the callback is sent to the Web API. (see my article on the event loop <a href="https://dev.to/BirendraMahto/javascript-visualized-event-loop-3dif">here</a>.)
...
First, we added a new property `name` to the proxy object (`person.name = "Birendra"`). `set` gets invoked, and logs `"Added a new property!"`.
...
```javascript
const person = { name: 'Birendra Mahto' };
...
###### 144. What do we need to add to the `person` object to get `["Birendra Mahto", 24]` as the output of `[...person]`?
...
Objects aren't iterable by default. An iterable is an iterable if the iterator protocol is present. We can add this manually by adding the iterator symbol `[Symbol.iterator]`, which has to return a generator object, for example by making it a generator function `*[Symbol.iterator]() {}`. This generator function has to yield the `Object.values` of the `person` object if we want it to return the array `["Birendra Mahto", 24]`: `yield* Object.values(this)`.
...
###### 153. What should the value of `method` be to log `{ name: "Birendra", age: 22 }`? 

```javascript
const keys = ["name", "age"]
const values = ["Birendra", 22]

const method = /* ?? */
Object[method](keys.map((_, i) => {
	return [keys[i], values[i]]
})) // { name: "Birendra", age: 22 }
```
...
This creates an array of subarrays containing the correct keys and values, which results in `{ name: "Birendra", age: 22 }`
